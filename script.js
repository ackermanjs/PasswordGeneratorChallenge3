...
function generatePassword() {
  ...
  numberOfCharacters = prompt(
    "Select a number of characters between 8 and 128."
  );
  ...
  hasUppercase = confirm(
    "Would you like your password to conatin uppercase characters?"
  );
  if (hasUppercase) {
    // Yes
  ...
  // Combine selected options (upper, lower, numbers, symbols) to create potentialResults
  if (hasLowercase) {
    potentialResults = potentialResults.concat(lowercaseCharacters);
  }
  if (hasUppercase) {
    potentialResults = potentialResults.concat(uppercaseCharacters);
  }
  if (hasNumbers) {
    potentialResults = potentialResults.concat(numbers);
  }
  if (hasSpecial) {
    potentialResults = potentialResults.concat(symbols);
  }
  // pick random cards out of new pool for length of password
  let finalResult = "";
  for (let i = 0; i < numberOfCharacters; i++) {
    let rng = [Math.floor(Math.random() * potentialResults.length)];
    // or finalfinalResult += potentialResults[rng];
    finalResult = finalResult + potentialResults[rng];
  }
  // Return the result as finalResult
  return finalResult;
}
